% pay attention to the patch,ncol,nrow,dt_mov,testmode,camera_bias,basepath,subfolder and line 86!
% only analyze one type of signal each time.
% dF/F analysis of neural activity in AOE experiment
% for HVI-Cy5 and other GEVIs,
% eg, fig 5b,5d
%% load and select
clear all; clc;
% Movie loading path
...
subfolder = '';
basepath = 'C:\Users\ZouOptics\Desktop\HVI code\MATLAB analysis\analysis in neuron\010253_opto-0.5VOD1 good\';
pathname = [basepath subfolder];
load ([pathname '\SNAPT-250ms blue light - 0.5V_4643.6034\matlab variables.mat']);%输入激光文件夹名称
wavelength = 488;%输入波长信息
% Fs = samprate;
% constants
movname = '\movie.bin';
ncol = 1024;         % x
