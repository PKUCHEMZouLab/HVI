...
%             
% Zoulab
% dF/F and Vm analysis of whole cell patch stimulated action potential, 
...
clear all; clc;
% Movie loading path
basepath = 'C:\Users\ZouOptics\Desktop\HVI code\MATLAB analysis\analysis in neuron\';
subfolder = '035841_300pA\'
pathname = [basepath subfolder '\']
a = importdata([pathname '\movie_DAQ.txt']);

if exist([basepath '\patch param.txt'])~= 0
...
load ([pathname '\Current injection_300pA-10ms-40 times_(Baseline 0 pA)\matlab variables.mat']);
% constants
% cycles = 80;
mode = 484;if mode == 484
    movname = '\movie.bin';
    ncol = 312;         % x
    nrow = 208;         % y
    camera_bias = 400;  % background due to camera bias (100 for bin 1x1)
    dt_mov = 2.0658;    % exposure time in millisecond (1058Hz)
    Fs = samprate;
else 
    movname = '\movie.bin';
    ncol = 176;         % x
...
figure();
plot(t_mov_selected,intens(:,1)-intens(:,2));
box off
axis tight
