function [roi_points,intens] = clicky_v2single(movie_in, dtmov, bin, refimg, bottom , top, titleofplot)
...
while(npts > 0)
    ...
    temp = single(repmat(inpoly, [1, 1, nframes]));
    itrace = squeeze(sum(sum(movie_in.*temp)))/sum(inpoly(:))-100*power(bin,2);
